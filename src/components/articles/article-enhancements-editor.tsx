...
import { Sparkles, Play, BookOpen, Book, Link as LinkIcon, Trash2, ExternalLink, CheckSquare, Square, X } from "lucide-react"
import { Card, CardContent, CardDescription, CardHeader, CardTitle } from "@/components/ui/card"
import { Button } from "@/components/ui/button"
import type { ArticleEnhancement } from "@/types/database"
import { AddEnhancementForm } from "./add-enhancement-form"
...
interface ArticleEnhancementsEditorProps {
  articleId: string
...
/**
 * Client-side wrapper for article enhancements with CRUD controls
 * Used in the dashboard for editing enhancements
 */
export function ArticleEnhancementsEditor({
  articleId,
  enhancements,
}: ArticleEnhancementsEditorProps) {
  ...
  const selectAllInGroup = (items: ArticleEnhancement[]) => {
    const newSelected = new Set(selectedIds)
      ...
      <div className="space-y-2">
        ...
        <div className="space-y-2">
          {items.map((enhancement) => {
            ...
            const metadata = enhancement.metadata as EnhancementMetadata | null
            return (
              <div
                key={enhancement.id}
              ...
              >
                ...
                {type === "youtube" && metadata?.thumbnail && (
                  <img
        ...
        {isSelectMode ? (
          ...
          <div className="flex items-center justify-between">
            <div>
              ...
              <CardDescription>
                {enhancements.length} enhancement(s) added
              </CardDescription>
            </div>
            <AddEnhancementForm articleId={articleId} onAdded={handleChange} />
          </div>
        )}
      </CardHeader>
      <CardContent>
        {enhancements.length > 0 ? (
          ...
          <p className="text-sm text-muted-foreground text-center py-8">
            No enhancements yet. Use the &quot;Enhance with AI&quot; button or add them manually.
          </p>
        )}
      </CardContent>
    </Card>
