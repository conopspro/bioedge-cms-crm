...
interface PresentationFormProps {
  ...
  existingPanelists?: PresentationPanelist[]
 ...
 *
 * Form for creating and editing presentations with AI-assisted content generation.
...
export function PresentationForm({
  ...
  const [recordingUrl, setRecordingUrl] = useState(presentation?.recording_url || "")
  const [recordingMetadata, setRecordingMetadata] = useState<any>(
    presentation?.recording_metadata || null
  )
  ...
  const [panelists, setPanelists] = useState<FormPanelist[]>(() => {
    ...
    if (presentation?.contact_id) {
      return [{
        ...
        article_id: presentation.article_id,
        display_order: 0,
      }]
    }
    return []
  ...
  const addPanelist = () => {
    ...
    setPanelists([
      ...
      {
        contact_id: "",
        role: panelists.length === 0 ? "moderator" : "panelist",
        company_id: null,
        article_id: null,
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    try {
      ...
      const url = isEditing ? `/api/presentations/${presentation.id}` : "/api/presentations"
      ...
      router.push(`/dashboard/presentations/${saved.id}`)
      router.refresh()
    } catch (err) {
    ...
    <form onSubmit={handleSubmit} className="space-y-6">
      ...
      <Card>
        <CardHeader>
          ...
          <CardDescription>
            Add a YouTube video recording for this session
          </CardDescription>
        </CardHeader>
        <CardContent className="space-y-4">
          <div className="space-y-2">
