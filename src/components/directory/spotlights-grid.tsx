...
interface SpotlightsGridProps {
  initialSpotlights: SpotlightItem[]
  initialHasMore: boolean
}

export function SpotlightsGrid({ initialSpotlights, initialHasMore }: SpotlightsGridProps) {
  const searchParams = useSearchParams()
  const category = searchParams.get("category")
  const search = searchParams.get("q")

  const [spotlights, setSpotlights] = useState<SpotlightItem[]>(initialSpotlights)
  ...
  }, [initialSpotlights, initialHasMore])
  ...
  const loadMore = useCallback(async () => {
    ...
    try {
      ...
      console.error("Failed to load more spotlights:", error)
    } finally {
      setIsLoading(false)
    }
    ...
    return (
      <div className="py-16 text-center">
        ...
        <h2 className="mb-2 text-xl font-semibold text-navy">
          {category || search ? "No spotlights found" : "No spotlights yet"}
        </h2>
