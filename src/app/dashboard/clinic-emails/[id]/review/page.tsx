...
export default function ClinicReviewQueuePage() {
  ...
  const saveEditsIfChanged = useCallback(async () => {
    ...
    try {
      await fetch(
        `/api/clinic-campaigns/${campaignId}/recipients/${current.id}`,
        {
          method: "PATCH",
  ...
  const handleRegenerate = async () => {
    ...
    try {
      const res = await fetch(
        `/api/clinic-campaigns/${campaignId}/recipients/${current.id}/regenerate`,
        { method: "POST" }
      )
      ...
      if (res.ok) {
        ...
        setQueue((prev) =>
          prev.map((r, i) =>
            i === currentIndex ? { ...r, subject: updated.subject, body: updated.body } : r
          )
        )
  ...
  const handleBulkDelete = async () => {
    ...
    try {
      // Delete one by one since we don't have a bulk delete endpoint for clinic recipients
      let deletedSoFar = 0
      for (const id of deleteSelectedIds) {
        const res = await fetch(
          `/api/clinic-campaigns/${campaignId}/recipients/${id}`,
          { method: "DELETE" }
        )
        if (res.ok) deletedSoFar++
      }

      setDeletedCount((prev) => prev + deletedSoFar)
      setQueue((prev) => prev.filter((r) => !deleteSelectedIds.has(r.id)))
      ...
      <div className="space-y-6">
        <div className="flex items-center gap-4">
          <Button variant="ghost" size="icon" asChild>
            <Link href={`/dashboard/clinic-emails/${campaignId}`}>
      ...
      <div className="space-y-6">
        <div className="flex items-center gap-4">
          <Button
            variant="ghost"
            size="icon"
            onClick={() => { setShowDeleteList(false); setDeleteSelectedIds(new Set()) }}
          >
            <ArrowLeft className="h-4 w-4" />
          </Button>
          <div className="flex-1">
            <h1 className="text-3xl font-bold tracking-tight">Select Emails to Delete</h1>
            <p className="text-muted-foreground">Check the emails you want to remove from this campaign.</p>
          </div>
        </div>
            ...
            >
              ...
              <div className="flex-1 min-w-0">
                <div className="flex items-center gap-2">
                  <span className="font-medium">{recipient.recipient_name || "Unknown"}</span>
        ...
        <div className="flex items-center gap-3">
          <Button variant="outline" onClick={() => { setShowDeleteList(false); setDeleteSelectedIds(new Set()) }}>
            Cancel
          </Button>
        ...
        <AlertDialog open={showBulkDeleteDialog} onOpenChange={setShowBulkDeleteDialog}>
          <AlertDialogContent>
            <AlertDialogHeader>
              ...
              <AlertDialogDescription>
                This will permanently remove {deleteSelectedIds.size} recipient{deleteSelectedIds.size > 1 ? "s" : ""} from this campaign. This cannot be undone.
              </AlertDialogDescription>
            </AlertDialogHeader>
            <AlertDialogFooter>
  ...
  return (
    <div className="space-y-6 max-w-3xl mx-auto">
      ...
      <div className="flex items-center gap-4">
        ...
        <div className="flex-1">
          <h1 className="text-3xl font-bold tracking-tight">Review Queue</h1>
          <p className="text-muted-foreground">{campaign?.name || "Campaign"}</p>
        </div>
      ...
      {/* Bulk Actions */}
      <div className="flex items-center gap-3">
        <Button variant="outline" onClick={() => setShowApproveAllDialog(true)}>
          <CheckCircle2 className="mr-2 h-4 w-4" />
          Approve All Remaining ({remainingCount})
        </Button>
        <Button variant="outline" onClick={() => setShowDeleteList(true)}>
        ...
        <Card>
          <CardHeader className="pb-3">
            <div className="text-sm">
              <span className="font-medium">To: {current.recipient_name || "Unknown"}</span>
              {current.recipient_email && (
                <span className="text-muted-foreground"> &lt;{current.recipient_email}&gt;</span>
              )}
            </div>
            <div className="text-sm text-muted-foreground">
              {current.clinic?.city && current.clinic?.state
                ? `${current.clinic.city}, ${current.clinic.state}`
                : ""}
              {current.clinic?.tags && current.clinic.tags.length > 0
                ? ` · ${current.clinic.tags.slice(0, 3).join(", ")}`
                : ""}
            </div>
          </CardHeader>
          ...
          <CardContent className="py-3">
            <div className="flex items-center gap-3">
              ...
              <Input
                ...
                type="email"
              />
              <Button size="sm" onClick={handleTestSend} disabled={sendingTest || !testEmail}>
                {sendingTest ? "Sending..." : "Send"}
              </Button>
              <Button size="sm" variant="ghost" onClick={() => { setShowTestSend(false); setTestResult(null) }}>
                <X className="h-4 w-4" />
              </Button>
            </div>
            {testResult && (
              <p className={`text-sm mt-2 ml-7 ${testResult.success ? "text-green-600" : "text-destructive"}`}>
                {testResult.message}
              </p>
            )}
          </CardContent>
      ...
      <div className="flex items-center justify-between">
        ...
        <div className="flex items-center gap-2">
          ...
          <Button variant="outline" onClick={() => { setShowTestSend(true); setTestResult(null) }}>
        ...
        <Button variant="outline" onClick={goNext} disabled={currentIndex >= queue.length - 1}>
          Next
        ...
        <div className="flex items-center gap-3">
          <div className="flex-1 h-2 rounded-full bg-muted overflow-hidden">
            <div
              ...
              style={{
                width: `${initialQueueSize.current > 0 ? Math.round(((approvedCount + deletedCount) / initialQueueSize.current) * 100) : 0}%`,
              }}
            />
          </div>
        </div>
      ...
      <AlertDialog open={showApproveAllDialog} onOpenChange={setShowApproveAllDialog}>
        <AlertDialogContent>
          <AlertDialogHeader>
            <AlertDialogTitle>Approve all {remainingCount} remaining emails?</AlertDialogTitle>
            <AlertDialogDescription>
              This will approve all {remainingCount} generated emails without further review. You can still edit individual emails from the campaign detail page.
            </AlertDialogDescription>
          </AlertDialogHeader>
          <AlertDialogFooter>
            <AlertDialogCancel>Cancel</AlertDialogCancel>
            <AlertDialogAction onClick={handleApproveAll} disabled={bulkApproving}>
