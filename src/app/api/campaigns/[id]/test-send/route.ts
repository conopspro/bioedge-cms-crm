 ...
 * No outreach_log entry, no status changes, no contact updates.
 *
...
export async function POST(request: NextRequest, { params }: RouteParams) {
  try {
    ...
    // Build email HTML with signature (same logic as send route)
    let emailHtml =
      recipient.body_html ||
    ...
    // NO side effects — no outreach_log, no status change, no contact update
