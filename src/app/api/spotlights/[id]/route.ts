  ...
  try {
    ...
    if (error || !data) {
      return NextResponse.json(
        { error: "Spotlight item not found" },
        { status: 404 }
      )
    }
    ...
    console.error("Spotlight get error:", error)
    return NextResponse.json(
...
export async function PATCH(request: NextRequest, { params }: RouteParams) {
  try {
    ...
    // Build update object for the spotlight
    ...
    if (updateError) {
      console.error("Failed to update spotlight item:", updateError)
      return NextResponse.json(
        { error: "Failed to update spotlight item" },
        { status: 500 }
      )
    }
    ...
    // Fetch the updated spotlight with all relations
...
export async function DELETE(request: NextRequest, { params }: RouteParams) {
  try {
    ...
    if (error) {
      console.error("Failed to delete spotlight item:", error)
      return NextResponse.json(
        { error: "Failed to delete spotlight item" },
        { status: 500 }
      )
    }
    ...
    console.error("Spotlight delete error:", error)
    return NextResponse.json(
      { error: "Internal server error" },
      { status: 500 }
    )
