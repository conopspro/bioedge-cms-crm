...
export async function GET(request: NextRequest) {
  try {
    ...
    // If filtering by contact, get spotlights where they're the primary speaker
    // OR where they're a panelist.
    if (contactId) {
      ...
      const panelistSpotlightIds = panelistSpotlights?.map(p => p.spotlight_id) || []

      if (panelistSpotlightIds.length > 0) {
        query = query.or(`contact_id.eq.${contactId},id.in.(${panelistSpotlightIds.join(",")})`)
      } else {
    ...
    console.error("Spotlights list error:", error)
    return NextResponse.json(
...
export async function POST(request: NextRequest) {
  try {
    ...
    if (spotlightError) {
      console.error("Failed to create spotlight item:", spotlightError)
      return NextResponse.json(
        { error: "Failed to create spotlight item" },
        { status: 500 }
      )
    }
    ...
    // Fetch the complete spotlight with panelists
    const { data: completeSpotlight } = await supabase
      .from("spotlights")
      .select(`
        *,
        contact:contacts(id, first_name, last_name, title, avatar_url),
    ...
    return NextResponse.json(completeSpotlight || spotlight, { status: 201 })
  } catch (error) {
    console.error("Spotlight create error:", error)
    return NextResponse.json(
      { error: "Internal server error" },
      { status: 500 }
    )
